...
def _read_counters(argv: list):
    p = subprocess.Popen(['sudo', './cache_query', *argv], close_fds=True, stdout=subprocess.PIPE)
    p.wait()
    out = p.stdout.read()
    if p.returncode != 0:
...
def main():
    ...
    for p in glob.glob('test/*.c'):
        ...
        with open('/tmp/perf.dat', 'w') as f:
            for variant in vars:
                counters = _read_counters(['/tmp/' + variant[0]])
                x_axis = int(variant[0].split('_')[1])
                y_axis = float(counters['cache_misses']) / float(counters['cache_references'])
                print('[*] %s%s%s: %d refs, %d misses (miss rate: %f)' % (ANSI_CYAN, variant[0],
                    ANSI_RESET, counters['cache_references'], counters['cache_misses'], y_axis))
                f.write('%d %f\n' % (x_axis, y_axis))
        ...
        os.rename('/tmp/plot.png', 'cache_query.png')
